"""Deep Deterministic Policy Gradient for Phase 2 logistics HNH.

The logistics simulator exposes a discrete hold-action API, but DDPG is a
continuous-action method. This agent learns a continuous hold duration in
minutes and maps it to the nearest simulator action index.
"""
...
class DDPGAgent:
    """Continuous-action DDPG adapted to Phase 2's discrete hold API."""
    ...
    def _raw_to_hold(self, raw: float, add_noise: bool = False) -> float:
        hold = (float(np.tanh(raw)) + 1.0) * 0.5 * self.action_high
        if add_noise:
            hold += float(self.noise.sample()[0]) * 5.0
        return float(np.clip(hold, self.action_low, self.action_high))

    def _hold_to_index(self, hold_min: float) -> int:
        return int(np.argmin([abs(hold_min - value) for value in self.HOLD_DURATIONS]))

    def select_action(self, state: np.ndarray) -> int:
        raw = self.actor.forward(state)
        hold_min = self._raw_to_hold(float(raw[0]), add_noise=True)
        return self._hold_to_index(hold_min)

    def greedy_action(self, state: np.ndarray) -> int:
        raw = self.actor.forward(state)
        hold_min = self._raw_to_hold(float(raw[0]), add_noise=False)
        return self._hold_to_index(hold_min)
    ...
    def _sync_targets(self, tau: float | None = None):
